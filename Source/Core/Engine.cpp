...
LRESULT UEngine::WndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_MOUSEWHEEL:
	{
		...
		for (FViewport* vp : UEngine::Get().GetWorld()->GetViewportManager()->GetViewports())
		{
			float curZoomSize = vp->GetClient()->GetOrthographicCamera()->GetZoomSize();
			vp->GetClient()->GetOrthographicCamera()->SetZoomSize(curZoomSize + zDelta);
		}
		break;
